...
def validate_name_prefix(prefix: str) -> tuple[bool, Optional[str]]:
    """Validate a name-prefix string.

    Returns ``(True, None)`` on success or ``(False, reason)`` on failure.
    """
    prefix = prefix.strip()
    if not prefix:
        return True, None  # empty prefix is allowed
    if len(prefix) > 100:
...
def validate_start_number(value: str) -> tuple[bool, Optional[str]]:
    """Validate that start-number looks like a phone."""
