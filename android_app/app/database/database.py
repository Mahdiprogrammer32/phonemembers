...
class Database:
    ...
    def _init_schema(self) -> None:
        with self._cursor() as cur:
            ...
            cur.execute("CREATE INDEX IF NOT EXISTS idx_contacts_created_by_app ON contacts(created_by_app)")
            ...
            for c in contacts:
                try:
                    cur.execute(
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?)""",
                        (c.internal_id, c.phone, c.generated_name, c.created_at, c.source, c.status, int(c.created_by_app)),
                    )
                    count += cur.rowcount
    ...
    def get_all_contacts(self, active_only: bool = True) -> list[Contact]:
        with self._cursor() as cur:
            if active_only:
                cur.execute("SELECT * FROM contacts WHERE status='active' ORDER BY created_at")
            else:
    ...
    def get_created_contacts(self) -> list[Contact]:
        with self._cursor() as cur:
            cur.execute("SELECT * FROM contacts WHERE created_by_app=1 AND status='active'")
            return [self._row_to_contact(row) for row in cur.fetchall()]
    ...
    def delete_created_contacts(self) -> int:
        with self._cursor() as cur:
            cur.execute("UPDATE contacts SET status='deleted' WHERE created_by_app=1 AND status='active'")
            return cur.rowcount
    ...
    def count_created_contacts(self) -> int:
        with self._cursor() as cur:
            cur.execute("SELECT COUNT(*) FROM contacts WHERE created_by_app=1 AND status='active'")
            return cur.fetchone()[0]

    def phone_exists(self, phone: str) -> bool:
        with self._cursor() as cur:
            cur.execute("SELECT 1 FROM contacts WHERE phone=? AND status='active'", (phone,))
    ...
    def insert_operation(self, op: Operation) -> None:
        with self._cursor() as cur:
            cur.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (op.operation_id, op.operation_type, op.total, op.processed, op.success, op.failed, op.skipped, op.status, op.created_at),
            )

    def update_operation(self, op: Operation) -> None:
        with self._cursor() as cur:
            cur.execute(
                """UPDATE operations SET total=?, processed=?, success=?, failed=?, skipped=?, status=?
                WHERE operation_id=?""",
                (op.total, op.processed, op.success, op.failed, op.skipped, op.status, op.operation_id),
            )
